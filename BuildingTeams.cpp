...
bool fg=true;
void dfs(int node,int parent,vector<vector<int>>&adj,vector<bool>&vis,vector<int>&team){
	vis[node]=true;
	for(auto it:adj[node]){
		if(it!=parent){
			if(!vis[it]){
				team[it]=!team[node];
				dfs(it,node,adj,vis,team);
			}
			else{
				if(team[node]==team[it]){
					fg=false;
					return;
				}
			}
