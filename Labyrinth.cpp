...
void solve() {
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(a[nx][ny]=='#' || (par[nx][ny]!=nvis)) continue;
            par[nx][ny]={x,y};
            q.push({nx,ny});
        }
        if(fg) break;
    ...
    while(curr!=end){
        ...
        int xdiff=nx-x;
        int ydiff=ny-y;
        if(xdiff==0 && ydiff==1) path.pb('R');
        else if(xdiff==0 && ydiff==-1) path.pb('L');
        else if(xdiff==1 && ydiff==0) path.pb('D');
        else if(xdiff==-1 && ydiff==0) path.pb('U');
        curr={nx,ny};
    }
    cout<<path.sz<<endl;
    cout<<path<<endl;
