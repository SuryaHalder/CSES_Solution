...
int f(int ind,int x,vi &c,vector<vector<int>>&dp){
	if(x<0) return 1e9;
	if(x==0) return 0;
	if(ind<0) return 1e9;
	if(dp[ind][x]!=-1) return dp[ind][x];
	int op1=1e9;
	if(x>=c[ind]) op1=1+f(ind,x-c[ind],c,dp);
	int op2=f(ind-1,x,c,dp);
	//cout<<min(op1,op2)<<endl;
	return dp[ind][x]=min(op1,op2);
}
void solve() {
    /*Your code here */
    int n,x;
