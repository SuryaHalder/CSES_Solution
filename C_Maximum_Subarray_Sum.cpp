...
void solve() {
    ...
    for(int i=ind-1;i>=0 && s[i]=='1';i--){
        curr+=a[i];
        lmaxi=max(curr,lmaxi);
    }
    curr=0;
    ll rmaxi=LLONG_MIN;
    for(int i=ind+1;i<n && s[i]=='1';i++){
        curr+=a[i];
        rmaxi=max(curr,rmaxi);
        //if(curr<0) curr=0;
    }
    if(lmaxi>=0 && rmaxi<=0) a[ind]=k-lmaxi;
    else if(lmaxi<=0 && rmaxi>=0) a[ind]=k-rmaxi;
    else if(lmaxi>=0 && rmaxi>=0) a[ind]=k-lmaxi-rmaxi;
    else a[ind]=k;
    yes;
    for(int i=0;i<n;i++){
        if(s[i]=='0' && i!=ind){
