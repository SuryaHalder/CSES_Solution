...
// int f(int node,vector<vector<int>>&adj,int &maxi){
//     if(adj[node].sz==0) return 0;
//     int l=0,r=0;
//     if(adj[node].sz>=1) l=f(adj[node][0],adj,maxi);
//     if(adj[node].sz>=2) r=f(adj[node][1],adj,maxi);
//     maxi=max(maxi,l+r);
//     return 1+max(l,r);
// }
void solve() {
    ...
    function<pair<int,int>(int)> bfs=[&](int s) -> pair<int,int>{
