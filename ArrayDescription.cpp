...
void solve() {
    ...
    // dp[ind][val]->no. of ways to fill a[ind] with value val
    if(a[0]==0){
    ...
    for(int ind=1;ind<n;ind++){
    	if(a[ind]==0){
    		for(int val=1;val<=m;val++){
    			for(auto it:{val-1,val,val+1}){
    				if(it>=1 && it<=m) dp[ind][val]=(dp[ind][val]+dp[ind-1][it])%mod;
    			}
    		}
    	}
    	else{
    		for(auto it:{a[ind]-1,a[ind],a[ind]+1}){
    			if(it>=1 && it<=m){
    				dp[ind][a[ind]]=(dp[ind][a[ind]]+dp[ind-1][it])%mod;
    			}
    		}
    	}
    }
