...
int f(int ind1,int ind2,string &s1,string &s2,vector<vector<int>>&dp){
    ...
    if(s1[ind1]==s2[ind2]) return dp[ind1][ind2]=f(ind1+1,ind2+1,s1,s2,dp);
    int op1=1+f(ind1+1,ind2+1,s1,s2,dp);
    int op2=1+f(ind1,ind2+1,s1,s2,dp);
    int op3=1+f(ind1+1,ind2,s1,s2,dp);
...
void solve() {
    ...
    string s1,s2;
    cin>>s1>>s2;
    vector<vector<int>>dp(s1.sz+1,vector<int>(s2.sz+1,-1));
    cout<<f(0,0,s1,s2,dp)<<endl;
}
